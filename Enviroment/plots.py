...
def plot_gaussian(ys, x_ga, y_ga, n, mu, sigma, X_test, grid, grid_min, part_ant):
    ...
    p1x = list(map(lambda x: x + abs(grid_min), int(part_ant[0])))
    p1y = list(map(lambda x: x + abs(grid_min), int(part_ant[1])))
    axs[0].plot(p1x, p1y, 'r')
    p2x = list(map(lambda x: x + abs(grid_min), int(part_ant[2])))
    p2y = list(map(lambda x: x + abs(grid_min), int(part_ant[3])))
    axs[0].plot(p2x, p2y, 'w')
    p3x = list(map(lambda x: x + abs(grid_min), int(part_ant[4])))
    p3y = list(map(lambda x: x + abs(grid_min), int(part_ant[5])))
    axs[0].plot(p3x, p3y, 'c')
    p4x = list(map(lambda x: x + abs(grid_min), int(part_ant[6])))
    p4y = list(map(lambda x: x + abs(grid_min), int(part_ant[7])))
