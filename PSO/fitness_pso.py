...
def part_fitness(secure, ok, x_h, y_h, fitness, g, GEN, xs, ys, part, s_ant, s_n, x_p, y_p, bench_function, y_data, n,
                 n_plot, n_data, grid_min, X_test, creator, best, df_bounds, part_ant, x_g, y_g, file, init=True):

    part, s_n = new_limit(g, xs, ys, part, df_bounds, part_ant, n_data, s_ant, s_n, secure)
    x_bench = int(part[0])
    y_bench = int(part[1])
