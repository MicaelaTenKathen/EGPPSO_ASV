...
c1, c2, c31, c41, length_scale, lam = 1, 1, 3, 4, 0.74, 0.20
...
for g in range(GEN):
    ...
    if (np.mean(distances) - last_sample) >= (np.min(post_array) * lam):
        if n_data == 1:
            if
            part_sample[0] = part[0]
            part_sample[1] = part[1]
        elif n_data == 2:
            part_sample[2] = part[0]
            part_sample[3] = part[1]
        elif n_data == 3:
            part_sample[4] = part[0]
            part_sample[5] = part[1]
        elif n_data == 4:
            part_sample[6] = part[0]
            part_sample[7] = part[1]
        c3 = c31
        c4 = c41
        k += 1
        ok = True
